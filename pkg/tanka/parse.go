...
func load(dir string, opts Opts) (*loaded, error) {
	_, env, err := eval(dir, opts.JsonnetOpts)
	if err != nil {
		return nil, err
	}
...
func eval(dir string, opts jsonnet.Opts) (raw interface{}, env *v1alpha1.Config, err error) {
	raw, env, err = evalJsonnet(dir, opts)
	if err != nil {
...
func evalJsonnet(baseDir string, opts jsonnet.Opts) (interface{}, *v1alpha1.Config, error) {
	var hasSpec bool
	specEnv, err := parseSpec(baseDir)
	if err != nil {
		switch err.(type) {
		case spec.ErrNoSpec:
			hasSpec = false
	...
	entrypoint, err := jpath.Entrypoint(baseDir)
	if err != nil {
		return nil, nil, err
	}
